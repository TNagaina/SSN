# -*- coding: utf-8 -*-
"""antiferoomagnetic_ising_model.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1xf-iju3nuB_B6qy1RlmRy9PVQL8ekbtV
"""
...
# Here we define the interactions of the model (2D spin Ising model, but antimagnetic: this means that the interaction constant when
#aplying a magnetic field is positive and that spins are favoured to be in opposite directions
...
def mcmove(config, beta):
    ...
    for i in range(N):
        for j in range(N):
                ...
                s =  config[a, b]
                #calculate energy cost of this new configuration (the % is for calculation of periodic boundary
                #condition): has to change form original model, now cost has a - in formt because it's contrary to the ferromagnetic model
...
#This function calculates the magnetization of a given configuration, where a staggered magnetitzation has (-1)^(i+j)
#that takes into account the alternating signs for antiferromagnetic materials:
def calcMag(config):
    '''Staggered magnetization for antiferromagnetic order'''
    N = len(config)
    staggered_mag = np.sum(config * np.fromfunction(lambda i, j: (-1) ** (i + j), (N, N), dtype=int))
...
plt.plot(T, Energy, 'o', color="plum");
plt.xlabel("Temperature (T)", fontsize=20);
plt.ylabel("Energy ", fontsize=20);

sp =  f.add_subplot(2, 2, 2 );
plt.plot(T, abs(Magnetization), 'o', color="turquoise");
plt.xlabel("Temperature (T)", fontsize=20);
plt.ylabel("Magnetization ", fontsize=20);

sp =  f.add_subplot(2, 2, 3 );
plt.plot(T, SpecificHeat, 'o', color="plum");
plt.xlabel("Temperature (T)", fontsize=20);
plt.ylabel("Specific Heat ", fontsize=20);

sp =  f.add_subplot(2, 2, 4 );
plt.plot(T, Susceptibility, 'o', color="turquoise");
plt.xlabel("Temperature (T)", fontsize=20);
